...
4. After the recharge is completed, if the `callbackUrl` field was provided in the [Create Payment API](/api/webapi/create-payment) request, a [Server-to-Server (S2S)](/guides/s2s) notification will be sent to the specified URL. The customer will then be redirected to the merchant's website URL provided in the request.
